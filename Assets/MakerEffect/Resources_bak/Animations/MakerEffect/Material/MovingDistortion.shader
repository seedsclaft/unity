...
Shader "EffekseerMaterial/MovingDistortion" {
...
Properties{
	...
	_Distortion_Tex("Color (RGBA)", 2D) = "white" {}
...
float val4= vcolor.w;
float val5=DistortionScale.x;
float2 val6=float2(0.5,0.5);
float2 val7=Speed.xy;
float2 val8=uv1+val7*_Time.y;
float4 val15 = ConvertFromSRGBTexture(tex2Dlod(_Distortion_Tex,float4(GetUV(val8),0,0)));
float3 val10=val15.xyz;
float4 val16_CompMask=float4(val10.x,val10.y,val10.z, 1.0);
float2 val16=val16_CompMask.xy;
float2 val17=(val16-val6);
float2 val18=(val17*float2(val5,val5));
float2 val19=uv1;
float2 val20=(val19+val18);
float4 val27 = ConvertFromSRGBTexture(tex2Dlod(_Color_Tex,float4(GetUV(val20),0,0)));
float val26=val27.w;
float3 val22=val27.xyz;
float val28=(val26*val4);
float3 val29=(val22*val0);
...
float2 val7=Speed.xy;
float2 val8=uv1+val7*_Time.y;
float4 val15 = ConvertFromSRGBTexture(tex2D(_Distortion_Tex,GetUV(val8)));
float3 val10=val15.xyz;
float4 val16_CompMask=float4(val10.x,val10.y,val10.z, 1.0);
float2 val16=val16_CompMask.xy;
float2 val17=(val16-val6);
...
float2 val19=uv1;
float2 val20=(val19+val18);
float4 val27 = ConvertFromSRGBTexture(tex2D(_Color_Tex,GetUV(val20)));
float val26=val27.w;
float3 val22=val27.xyz;
float val28=(val26*val4);
float3 val29=(val22*val0);
