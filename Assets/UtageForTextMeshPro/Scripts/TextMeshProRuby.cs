...
namespace Utage
{
	...
	[AddComponentMenu("Utage/TextMeshPro/Ruby")]
	public class TextMeshProRuby : MonoBehaviour
	{
		...
		List<int> VisivleIndexList { get { return visivleIndexList; } }
		List<int> visivleIndexList = new List<int>();
		...
		{
			int visible = 0;
			for (int i = 0; i < VisivleIndexList.Count; ++i)
			{
				if (index < VisivleIndexList[i])
				{
					break;
				}
				visible = i + 1;
...
//			Debug.LogFormat("{2} rubyWidth = {0} stringWidth = {1}", rubyWidth, stringWidth, text);
			...
			else
			{
				...
				float space = (TextWidth - RubyWidth) / text.Length / 2;
