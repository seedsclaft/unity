...
public class ResourcesController 
{
    ...
    static void Deploy() {
        var skills = Resources.Load<SkillsData>("Data/Skills");
        var states = Resources.Load<StatesData>("Data/States");
        var strs = new List<string>();
        foreach (var item in skills._data)
        {
            if (item.AnimationName != "")
            strs.Add(item.AnimationName);
        }
        foreach (var item in states._data)
        {
            if (item.EffectPath != "")
